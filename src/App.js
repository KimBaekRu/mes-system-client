                      ...
                      <React.Fragment key={`main-${mainM.originalIndex}`}>
                        ...
                        {((!isAdmin) || (isAdmin && isEditMode)) && openMaintInputIdx === mainM.originalIndex && (
                          <li style={{ marginBottom: 2, display: 'flex', alignItems: 'center', background: '#eef', padding: 4, borderRadius: 4 }}>
                            <input type="text" placeholder="시작" value={plusInputs[mainM.originalIndex]?.start || ''} onChange={e => setPlusInputs(inputs => ({ ...inputs, [mainM.originalIndex]: { ...inputs[mainM.originalIndex], start: e.target.value } }))} style={{ width: 70, marginRight: 6, color: '#000' }} />
                            <span style={{ marginRight: 6, color: '#000' }}>~</span>
                            <input type="text" placeholder="종료" value={plusInputs[mainM.originalIndex]?.end || ''} onChange={e => setPlusInputs(inputs => ({ ...inputs, [mainM.originalIndex]: { ...inputs[mainM.originalIndex], end: e.target.value } }))} style={{ width: 70, marginRight: 6, color: '#000' }} />
  ...
  const updateAssignmentTableStatus = (equipmentName, status) => {
    ...
    setAssignmentTables(prevTables => {
      return prevTables.map(table => {
        ...
        if (table.statusCells && table.statusCells.length > 0 && titleLower.includes(equipmentLower)) {
            const newData = table.data.map((row, rowIndex) => {
              return row.map((cell, colIndex) => {
                const cellKey = `${rowIndex}-${colIndex}`;
                  ...
                  {row.map((cell, colIndex) => {
                    ...
                    const getCellBackgroundColor = () => {
                      ...
                      if (statusCells.includes(cellKey)) {
                        ...
                        if (cell === '가동대기') return 'rgba(255, 255, 0, 0.3)'; // 노란색 - 버튼과 일치
