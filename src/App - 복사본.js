...
// 공정명(타이틀) 노드 컴포넌트
function ProcessTitleNode({ title, x, y, id, isAdmin, onMove, onEdit, onDelete, onClick, yieldValue, setYieldValue, secondValue, setSecondValue, lineName, maintenanceHistory, onAddMaint, onDeleteMaint, showMaint, setShowMaint, zIndex, lastSaved: propLastSaved, equipments, setEquipments }) {
  console.log('App.js ProcessTitleNode render - id:', id, 'secondValue:', secondValue);
  ...
  const handleSaveYield = () => {
    ...
    setSecondValue(secondInputValue); // secondInputValue도 저장
    setLastSaved(lastSavedStr);
    fetch(`http://localhost:3001/api/processTitles/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ yield: inputValue, secondField: secondInputValue, lastSaved: lastSavedStr })
    });
  };
  // 생산량 입력 상태 별도 관리
  ...
  // 추가값 입력 상태 별도 관리
  const [secondInputValue, setSecondInputValue] = useState(secondValue || '');
  useEffect(() => { setSecondInputValue(secondValue || ''); }, [secondValue]);
  // 자재명 입력 상태 추가
  ...
  // 생산량 입력창 바깥 클릭 시 닫힘 처리 (파란 테두리 전체 기준)
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showMaint && (
          <div ref={blueBoxRef} style={{ background: '#f9f9f9', border: '1px solid #ccc', padding: 6, marginTop: 2, fontSize: 11, minWidth: 260, color: '#000' }} onClick={e => e.stopPropagation()}>
            {/* 생산량 입력란을 두 개로 나누기 */}
            <div style={{ marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center', color: '#000' }}>
              ...
              <input
                ...
                readOnly={isAdmin}
              />
              <span style={{ color: '#000', marginLeft: 8 }}>자재명:</span>
              <input
                type="text"
                inputMode="numeric"
                value={secondInputValue}
                onChange={e => {
                  if (!isAdmin) setSecondInputValue(e.target.value);
                }}
                style={{
                  minWidth: 60,
                  width: 60,
            ...
            {/* {materialNames.length > 0 && (
              <div style={{ color: '#0074D9', fontWeight: 'bold', fontSize: 13, marginBottom: 4, textAlign: 'left', display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                자재명:
                {materialNames.map((name, idx) => (
                  <span key={idx} style={{ background: '#e3f2fd', color: '#0074D9', borderRadius: 4, padding: '1px 6px', marginRight: 4, display: 'inline-flex', alignItems: 'center', fontWeight: 'bold', fontSize: 13 }}>
                    {name}
                    <button
                      style={{ marginLeft: 4, color: '#d00', background: 'none', border: 'none', fontWeight: 'bold', fontSize: 13, cursor: 'pointer', lineHeight: 1 }}
                      onClick={() => setMaterialNames(names => names.filter((_, i) => i !== idx))}
                      title="삭제"
                      type="button"
                    >×</button>
                  </span>
...
function EquipmentNode({ eq, onMove, onDelete, onStatusChange, isAdmin, equipments, setEquipments, showStatus, setShowStatus, onClick, zIndex, optionInputOpen, showMaint, setShowMaint, showMemo, setShowMemo, openPopup, setOpenPopup, showOptionBox, setShowOptionBox, openOptionEquipmentId, setOpenOptionEquipmentId, resizeTargetId, setResizeTargetId }) {
  ...
  return (
    <Draggable
      ...
      key={eq.id + '-' + eq.x + '-' + eq.y}
    >
      <div style={{ position: 'absolute', width: 80, zIndex }}>
        {/* 메모 세모 마크 */}
        ...
        <div
          ...
          onClick={e => {
            ...
            setShowStatus(false); // 상태창 닫힘
            setResizeTargetId(null); // 장비 이름 클릭 시 이모티콘 숨김
          }}
        >
          {isAdmin && edit ? (
            <input
        ...
        {processTitles.map((item) => (
          <ProcessTitleNode
            ...
            onClick={e => {
              ...
              setOpenOptionEquipmentId(null); // 장비 옵션창 닫기
              setOpenStatusEquipmentId(null); // 장비 상태창 닫기
              setResizeTargetId(null);        // 리사이즈 핸들 닫기
              setOpenPopup(null);             // 메모/생산량/정비 팝업 닫기
            }}
            ...
            setShowMaint={show => setOpenPopup(show ? { type: 'maint', id: item.id } : null)}
