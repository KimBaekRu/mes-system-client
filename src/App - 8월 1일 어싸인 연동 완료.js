                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      .then(updated => {
                        ...
                        setTimeout(() => {
                        updateAssignmentTableMaterial(eq.name, updated.options || []);
                      ...
                      <button style={{ marginLeft: 2, fontSize: 11, color: '#d00', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 'bold', lineHeight: 1 }} onClick={e => {
                        ...
                        if (window.confirm(`"${opt}" 자재명을 삭제하시겠습니까?`)) {
                          ...
                          .then(updated => {
                            ...
                            setTimeout(() => {
                            updateAssignmentTableMaterial(eq.name, newOptions);
...
function DraggableAssignmentTable({ table, onUpdateTable, onDeleteTable }) {
  ...
  const addRow = () => {
    ...
    // 높이 자동 재계산
    const headerHeight = 25;
    const controlHeight = isEditMode ? 25 : 0;
    const rowHeight = 24;
    const padding = 8;
    const contentHeight = headerHeight + controlHeight + (newData.length * rowHeight) + padding;
    const newAutoHeight = Math.min(Math.max(contentHeight, 55), 300); // 최소 높이를 55로 수정
    
    onUpdateTable({ ...table, data: newData, height: newAutoHeight });
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ 
          overflow: 'auto', 
          height: `calc(${autoHeight}px - ${isEditMode ? 60 : 35}px)`, // 자동 높이에 맞게 조정
          padding: '4px'
        }}>
          <table style={{
            width: '100%',
