...
function ProcessTitleNode({
  ...
  return (
    <Draggable defaultPosition={{ x, y }} onStop={(e, data) => isAdmin && onMove(id, data.x, data.y)} disabled={!isAdmin} key={id + '-' + x + '-' + y}>
      <div style={{ position: 'absolute', zIndex: zIndex || 1, minWidth: 60, background: '#0074D9', color: '#fff', border: '1px solid #bbb', borderRadius: 4, padding: '2px 8px', textAlign: 'center', boxShadow: '0 1px 4px #ccc', fontWeight: 'bold', fontSize: 14 }} onClick={e => { if (onClick) onClick(e); }}>
        ...
        <button style={{ marginLeft: 6, fontSize: 10 }} onClick={e => { e.stopPropagation(); if (typeof onClick === 'function') onClick(e); setShowMaint(s => !s); }}>생산량</button>
        {showMaint && (
          <div ref={blueBoxRef} style={{ position: 'relative', zIndex: 9999, background: '#f9f9f9', border: '1px solid #ccc', padding: '12px', marginTop: 2, fontSize: 11, color: '#000', minWidth: 'max-content', textAlign: 'left' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginTop: '16px' }}>
              ...
              <ul style={{ paddingLeft: 12, margin: 0 }}>
                {(() => {
                  // 1. 블럭 내 이력 중에서 장비명(eqNo)별로 그룹핑
                  const blockMaints = (Array.isArray(maintenanceHistory) ? maintenanceHistory : [])
                    .map((m, originalIndex) => ({ ...m, originalIndex }))
                    .filter(m => m.blockIndex === index || (m.blockIndex === undefined && index === 0));
                  const eqNoMap = {};
                  blockMaints.forEach(m => {
                    const eqNoKey = m.eqNo || '미지정';
                    if (!eqNoMap[eqNoKey]) eqNoMap[eqNoKey] = [];
                    eqNoMap[eqNoKey].push(m);
                  });
                  // 2. 장비명별로 이력+정비내역별 그룹핑+총시간+추가입력창 렌더링
                  return Object.entries(eqNoMap).map(([eqNoKey, maints]) => {
                    // 정비내역(종류)별로 그룹핑 (description의 마지막 단어 기준)
                    const typeMap = {};
                    maints.forEach(m => {
                      // description에서 정비내역 종류 추출 (마지막 단어)
                      let desc = m.description.replace(/\d{2}:\d{2}~\d{2}:\d{2}\s*/, '').trim();
                      let type = desc.split(/\s+/).pop() || '미지정';
                      if (!typeMap[type]) typeMap[type] = [];
                      typeMap[type].push(m);
                    });
                    return (
                      <React.Fragment key={eqNoKey}>
                        {/* 장비명 헤더 */}
                        <li style={{ fontWeight: 'bold', color: '#006400', fontSize: 14, marginTop: 4, marginBottom: 2 }}>
                          [{eqNoKey}]
                        </li>
                        {/* 정비내역(종류)별 그룹 */}
                        {Object.entries(typeMap).map(([type, groupMaints], groupIdx) => {
                          // 총 정비 시간 계산
                          ...
                          return (
                            <React.Fragment key={type + groupIdx}>
                              {/* 이력 리스트 */}
                              {groupMaints.map((m, j) => {
                                let timeRange = '';
                                let minText = '';
                                ...
                                return (
                                  <li key={m.originalIndex + '-' + j} style={{ marginBottom: 2, display: 'flex', alignItems: 'center' }}>
                                    <span style={{ flex: 1, textAlign: 'left', display: 'block' }}>
                                      ...
                                      {m._addedByPlus && (<span style={{ color: '#0074D9', marginLeft: 4, fontWeight: 'bold', fontSize: 11 }}>(추가)</span>)}
                                    </span>
                                    <button style={{ marginLeft: 6, fontSize: 10 }} onClick={() => onDeleteMaint(id, m.originalIndex)}>삭제</button>
                                    {!isAdmin && (<button style={{ marginLeft: 2, fontSize: 10 }} onClick={() => setOpenMaintInputIdx(openMaintInputIdx === m.originalIndex ? null : m.originalIndex)}>+</button>)}
                                  </li>
                                );
                              })}
                              {/* eqNo+정비내역별로 +버튼 눌렀을 때 추가입력창 */}
                              {!isAdmin && groupMaints.some(m => openMaintInputIdx === m.originalIndex) && (
                                <li style={{ marginBottom: 2, display: 'flex', alignItems: 'center', background: '#eef', padding: 4, borderRadius: 4 }}>
                                  <input type="text" placeholder="시작(예: 08:00)" value={plusInputs[openMaintInputIdx]?.start || ''} onChange={e => setPlusInputs(inputs => ({ ...inputs, [openMaintInputIdx]: { ...inputs[openMaintInputIdx], start: e.target.value } }))} style={{ width: 70, marginRight: 6, color: '#000' }} />
                                  <span style={{ marginRight: 6, color: '#000' }}>~</span>
                                  <input type="text" placeholder="종료(예: 08:05)" value={plusInputs[openMaintInputIdx]?.end || ''} onChange={e => setPlusInputs(inputs => ({ ...inputs, [openMaintInputIdx]: { ...inputs[openMaintInputIdx], end: e.target.value } }))} style={{ width: 70, marginRight: 6, color: '#000' }} />
                                  <input type="text" placeholder="정비 내용" value={plusInputs[openMaintInputIdx]?.desc || ''} onChange={e => setPlusInputs(inputs => ({ ...inputs, [openMaintInputIdx]: { ...inputs[openMaintInputIdx], desc: e.target.value } }))} style={{ flexl: 1, marginRight: 6, color: '#000' }} />
                                  <button style={{ fontSize: 10, padding: '2px 8px' }} onClick={() => {
                                    const s = plusInputs[openMaintInputIdx]?.start || '';
                                    const e_ = plusInputs[openMaintInputIdx]?.end || '';
                                    const d = plusInputs[openMaintInputIdx]?.desc || '';
                                    const eqNo = eqNoKey;
                                    if (!s || !e_ || !d || !eqNo) {
                                    ...
                                    if (isNaN(min)) min = '';
                                    if (min < 0) min += 24 * 60;
                                    // description은 기존 그룹 type(정비내역 종류)로 맞춰줌
                                    const newMaint = {
                                      time: min,
                                      description: `${s}~${e_} ${d}`,
                                      eqNo,
                                      _addedByPlus: true,
                                      blockIndex: index,
                                      _groupSource: openMaintInputIdx
                                    };
                                    onAddMaint(id, newMaint);
                                    setPlusInputs(inputs => ({ ...inputs, [openMaintInputIdx]: { start: '', end: '', desc: '' } }));
...
function EquipmentNode({ eq, onMove, onDelete, onStatusChange, isAdmin, equipments, setEquipments, showStatus, setShowStatus, onClick, zIndex, optionInputOpen, showMaint, setShowMaint, showMemo, setShowMemo, openPopup, setOpenPopup, showOptionBox, setShowOptionBox, openOptionEquipmentId, setOpenOptionEquipmentId, resizeTargetId, setResizeTargetId }) {
  ...
  return (
    <Draggable
      ...
      key={eq.id + '-' + eq.x + '-' + eq.y}
    >
      <div style={{ position: 'absolute', width: 80, zIndex: zIndex || 1 }} onClick={e => { if (onClick) onClick(e); }}>
        {/* 메모 세모 마크 */}
        ...
        <div
          onClick={e => {
            e.stopPropagation();
            setShowStatus(true); // 상태 옵션창만 열림
            setOpenOptionEquipmentId(null); // 자재명 옵션창은 무조건 닫힘
            if (isAdmin) setResizeTargetId(eq.id);
          }}
          onDoubleClick={e => {
        ...
        >
          ...
          {resizing && (
            ...
            />
              ...
              {/* 저장/취소/되돌리기 버튼 (리사이즈 모드일 때만) */}
              <div style={{ position: 'absolute', left: '50%', top: '-60px', transform: 'translateX(-50%)', zIndex: 30, background: '#fff', border: '1px solid #bbb', borderRadius: 4, padding: '2px 6px', display: 'flex', flexDirection: 'row', gap: 4, boxShadow: '0 2px 8px #bbb', minHeight: 0, minWidth: 0 }}>
        ...
        <div
          ...
          onClick={e => {
            e.stopPropagation();
            setOpenOptionEquipmentId(eq.id); // 자재명 옵션창만 열림
            setShowStatus(false); // 상태 옵션창은 무조건 닫힘
        ...
        {/* 자재명 입력/추가: 장비 이름 클릭 시에만, openOptionEquipmentId === eq.id && !showMemo일 때만 */}
        {openOptionEquipmentId === eq.id && !showMemo && (
          isAdmin ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', marginTop: 4, gap: 4, width: (pendingSize ? pendingSize.width : imgSize.width), margin: '0 auto', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, marginTop: 4, width: '100%', textAlign: 'center' }}>
                {options.map(opt => (
                  <span key={opt} style={{ display: 'flex', alignItems: 'center', background: '#eee', color: '#333', borderRadius: 3, padding: '1px 6px', fontSize: 11, margin: '1px 0', marginLeft: 'auto', marginRight: 'auto' }}>
                    {opt}
            ...
            options.length > 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 4, width: (pendingSize ? pendingSize.width : imgSize.width), margin: '0 auto', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
                <select
                  value={selectedOption}
                  onChange={handleSelectOption}
                  style={{ fontSize: 12, padding: '2px 6px', borderRadius: 4, minWidth: 60, maxWidth: '100%', width: 'auto', margin: '0 auto', textAlign: 'left' }}
                >
                  {options.map(opt => (
                ...
                {selectedOption && (
                  <div style={{ color: '#00e676', fontWeight: 'bold', fontSize: 13, marginTop: 2 }}>{selectedOption}</div>
                )}
              </div>
            )
          )
