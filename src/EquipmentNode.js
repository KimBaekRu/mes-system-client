...
export default function EquipmentNode({ eq, onMove, onDelete, onStatusChange, isAdmin, equipments, showStatus, setShowStatus, onClick, zIndex, optionInputOpen, showMaint, setShowMaint, showMemo, setShowMemo, openPopup, setOpenPopup, showOptionBox, setShowOptionBox }) {
  const statusOptions = [
  ...
  useEffect(() => { setValue(eq.name); }, [eq.name]);
  const saveName = () => {
    setEdit(false);
    if (value !== eq.name) {
  ...
  useEffect(() => { setMemo(eq.memo || ''); }, [eq.memo]);
  ...
  return (
    ...
    >
      <div 
        ...
        onClick={e => { onClick(e); setShowOptionBox(true); }}
        onMouseEnter={e => {
      ...
      >
        ...
        <div style={{ width: '100%', height: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 2 }}>
          <div style={{
            width: 8,
            height: 8,
            borderRadius: '50%',
            background: `radial-gradient(circle at 30% 30%, ${lampColor[eq.status] || 'gray'}, ${lampColor[eq.status] || 'gray'})`,
        ...
        <div
          ...
          style={{ width: '100%', height: 48, cursor: 'pointer', position: 'relative' }}
        >
          {eq.iconUrl ? (
            <img 
              src={eq.iconUrl} 
              ...
              onMouseEnter={e => {
                e.target.style.transform = 'perspective(100px) rotateX(0deg) scale(1.05)';
              ...
              onMouseLeave={e => {
                e.target.style.transform = 'perspective(100px) rotateX(2deg) scale(1)';
                e.target.style.boxShadow = `
                  0 8px 16px rgba(0,0,0,0.15),
                  0 4px 8px rgba(0,0,0,0.1),
                  inset 0 1px 0 rgba(255,255,255,0.8),
        ...
        <div style={{ width: '100%', textAlign: 'center', fontWeight: 'bold', fontSize: 13, marginTop: 2, color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', textShadow: '0 1px 2px #222' }}>
          {eq.name}
        </div>
        {showOptionBox ? (
          isAdmin ? (
            <div style={{ width: '100%', textAlign: 'center', marginTop: 2 }}>
              ...
              <div style={{ marginTop: 2 }}>
                {options.map(opt => (
                  <span key={opt} style={{ display: 'inline-block', background: '#eee', color: '#333', borderRadius: 4, padding: '1px 6px', fontSize: 11, margin: '0 2px' }}>
                    {opt}
          ...
          selectedOption && (
            <div style={{ width: '100%', textAlign: 'center', fontSize: 12, color: '#00e676', fontWeight: 'bold', marginTop: 2 }}>
              {selectedOption}
            </div>
          )
        )}
