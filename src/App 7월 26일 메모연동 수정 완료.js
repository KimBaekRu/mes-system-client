...
function EquipmentNode({ eq, onMove, onDelete, onStatusChange, isAdmin, isEditMode, equipments, setEquipments, showStatus, setShowStatus, onClick, zIndex, optionInputOpen, showMaint, setShowMaint, showMemo, setShowMemo, openPopup, setOpenPopup, showOptionBox, setShowOptionBox, openOptionEquipmentId, setOpenOptionEquipmentId, resizeTargetId, setResizeTargetId, showPopup, setPopups, currentTeam }) {
  ...
  const saveMemo = () => {
    ...
    setEquipments(eqs => eqs.map(e => e.id === eq.id ? { ...e, memo } : e));
  };
    ...
    >
      <div style={{ position: 'absolute', width: 80, zIndex }} data-equipment-id={eq.id}>
        ...
        >
          ...
          {showStatus && ((!isAdmin) || (isAdmin && isEditMode)) && !showMemo && (
            <button
              style={{
                position: 'absolute',
                top: -10,    // 더 위로
...
export default function App() {
  ...
  const addProcessMaint = (processId, newMaint) => {
    ...
    if (newMaint.eqNo) {
      ...
      console.log(`장비 "${eqName}" 검색 결과: ${matchedEqs.length}개 매칭`);
  ...
  const deleteProcessMaint = (processId, indexToDelete) => {
    ...
    if (deletedMaint && deletedMaint.eqNo) {
      ...
      setEquipments(eqs =>
        eqs.map(e =>
          e.name === eqName
            ? { ...e, memo: localStorage.getItem(`equipment_${e.id}_memo_${currentTeam}`) || '' }
            : e
        )
      );
    }
    ...
    <>
      ...
      <div style={{ marginBottom: 12 }}>
        <button onClick={() => {
          ...
          setResizeTargetId(null);
          setIsAdmin(a => !a);
          setIsEditMode(false); // 모드 전환 시 편집 모드 해제
      ...
      <div style={{ marginBottom: 12, textAlign: 'center' }}>
        <span style={{ marginRight: 10, fontWeight: 'bold', fontSize: 16 }}>조 선택:</span>
        {['A', 'B', 'C'].map(team => (
          <button
            key={team}
            ...
            style={{
              ...
              fontWeight: 'bold',
              fontSize: 14,
              backgroundColor: currentTeam === team ? '#2196F3' : '#f0f0f0',
              color: currentTeam === team ? 'white' : '#333',
              border: currentTeam === team ? '2px solid #1976D2' : '2px solid #ccc',
              borderRadius: '8px',
              cursor: 'pointer'
            }}
          >
        ...
        <span style={{ marginLeft: 15, color: '#666', fontWeight: 'bold' }}>
          현재: {currentTeam}조
        </span>
      </div>
      
      {isAdmin && isEditMode && <AddEquipmentForm onAdd={addEquipment} />}
      {/* 공정명(타이틀) 추가 폼 - 편집 모드에서만 */}
      {isAdmin && isEditMode && (
